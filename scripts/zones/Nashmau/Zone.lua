...
function onZoneIn(player,prevZone)
  ...
  if ((player:getXPos() == 0) and (player:getYPos() == 0) and (player:getZPos() == 0)) then 
         if (prevZone == 54 and player:getCurrentMission(TOAU) == THE_BLACK_COFFIN and player:getVar("AhtUrganStatus") == 3) then  -- zone in after defeating The Black Coffin
            cs = 281;
...
function onEventFinish(player,csid,option)
   if (csid == 0x00c8) then  
    ...
    elseif (csid == 281) then
        player:completeMission(TOAU,THE_BLACK_COFFIN);     -- End of Black Coffin Mission
